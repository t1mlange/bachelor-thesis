...
\begin{document}
    ...
    For example, implementing the \code{notifyMethodChanged()} method for the use inside StubDroid.
    ...
    Because the assignment type is a heap type, the backward analysis now recognizes that it possibly missed updates to \code{b.str} below of line 2.
    ...
    The main analysis leaves the incoming taint \code{b.str} untouched but notices \code{a} aliases \code{b} below line 2, hence invoking the alias analysis with \code{a.str}.
    ...
    The introduced taint \code{str} in line 6 also initially has line 6 as a turn unit. 
    ...
    \begin{itemize}
        ...
        \item \code{Unit getConditionalBranchIntraprocedural(Unit unit)}
    \end{itemize}
    ...
    The second one is used at return edges and returns the conditional statement if the call site is inside a conditional else \code{null}. 
    ...
    As taints are introduced at sources, if the source statement is a static initialization as shown in \autoref{lst:clinit_examples_a}, the propagation starts inside the \code{<clinit>} method. 
