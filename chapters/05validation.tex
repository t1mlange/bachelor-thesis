...
\begin{document}
    \chapter{Validation}
    \section{Unit Tests}
    \textsc{FlowDroid} already contains 519 unit tests for the core component. We also validated the backwards analysis with these tests with good results. Because we focused on the context-sensitive analysis, not all analysis-related tests were applicable. In the following, we briefly explain why tests were left out or did not return the same results.
    ...
    For the validation we ran \textsc{FlowDroid} with the default configuration of the Android module using the Easy Taint Wrapper as the taint wrapper. The configuration is summarized in \autoref{t:droidbenchconfig}.
    ...
    \subsection{Results}
    The full overview of the results is in \autoref{t:droidbenchvalidation}. 
    ...
    Our backwards-directed implementation yields nearly the same result as the existing forwards implementation with one missed leak more than the baseline. We achieve a F1 measure of $0.89$ which is the same as the baseline.
    ...
    This is a limitiation of the alias analysis where only encountered aliases are tracked. Tracking more aliases is too inefficient for the analysis to be applicable and also fully precise aliasing is still an unsolved problem \cite{Bodden2012}.
