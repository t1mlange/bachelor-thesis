...
\begin{document}
    ...
    Most test cases are small enough to be analysed in sub-two seconds on an average four core desktop CPU from 2012. Our test environment is not isolated, so background tasks and the process scheduler can affect the runtime. The short runtime together with the variance of the unisolated testing environment render the runtime unusable as an comparison point. In constrast, edge propagations are deterministic\footnote{This is only true if there are enough resources. \textsc{FlowDroid} tries to terminate gracefully when running low on memory. Also, timeouts result in a non-reproducible number of edge propagations.} and correlate with the runtime. Thus, we only use the number of propagations to compare both implementations.

    The configuration is the same as described in \autoref{s:droidbenchconfig}.

    \subsection{Results}
    We compare all test cases where both implementations yield the same result. Rows containing just hyphens are either test cases where the result differs or the IFDS analysis did not start, e.g. because no sink was found in the reachable code. \#I denotes the number of edge propagations inside the infoflow analysis and \#A the number of edge propagations inside the alias analysis. The absolute difference columns calculate from $\mathit{Result}_{\mathit{B}} - \mathit{Result}_{\mathit{F}}$. The relative difference is calculated as follows: $\frac{\mathit{TotalDifference}}{|\mathit{\#I_\mathit{F} + \#A_\mathit{F}}|}$. Hence, negative values signify the backwards analysis performed better. The full results can be found in \autoref{t:droidbenchevaluation}.

    In general, both implementations have similiar average edge propagation counts. There are not many test cases where both perform identical, instead dependent on the specific test case the relative difference is in between $-1$ and $1$. So, the expected behavior from \autoref{s:complexity} occured: it highly depends on the analyzed app.
    But what we did not expect is some sort of condition which lets the backwards edge propagations explode up to a factor of $10 000\%$ as seen in \code{LifecycleTest#BroadcastReceiverLifecycle3} and others. In contrast, the existing forwards implementation only at most $100\%$ relative difference.
    ...
    This test contains two sinks and one source. The tainted data is written to a file, later on read from the file and leaked. \textsc{FlowDroid} does not support tainting over files, so it only finds a leak from source to file write but misses the leak from file read to send SMS. 
    Backwards, due to EasyTaintWrapper's simplicity overtainting happens. When \code{FileInputStream fis = openFileInput("out.txt");}
    is called with \code{fis} tainted, EasyTaintWrapper taints also the base object - in this case the MainActivity. As the MainActivity has a large scope, the taint has a long lifetime and many other taints are derived from this taint. This explains the relative difference of $17.68$.
    ...
    Two \code{LocationListener}s are registered on the location in different activities. In both activities, an instance field is a parameter of a sink.
    So there are two possible paths were something could be leaked. In the first path, the LocationListener does not call any source. The second path has an empty setter method killing the taint.
    For the first path, the backwards analysis has to visit all the way into the \code{LocationListener} to notice that this is a dead end while the forwards search does not even start there.
    For the second path, the backwards analysis seem to suffer because it starts with a instance field tainted which has a larger scope than a local.

    \paragraph{BroadcastReceiverLifecycle3}
    The test contains five sinks but only one source. If we only consider the leak path, both implementations perform equal. 
    The four other sinks are responsible for the overhead on edge propagations.
    
    \paragraph{Reflection6}
    The reflective call site has multiple callees in the interprocedural control-flow graph. Backwards all of these callees are visited of which only one contains a source statement. Forwards the taint is introduced in the callee at the source and just one return site needs to be processed.

    \section{Real World Apps}

    \subsection{Configuration}
    Our test machine is equipped with four Intel Xeon E5-4650 and 1 TB of RAM. We limited the JVM to 50 GB RAM and \textsc{FlowDroid} on 16 threads per instance. We ran at most four instances in parallel to ensure a one-to-one mapping between CPU threads and \textsc{FlowDroid} threads. Note that the test machine is a shared system but we made sure there are always enough resources for our evaluation available. Still, background services might influence the performance of a single run. To stamp out this factor each app ran three times and only the median is used. If there were outliers we repeated the runs.
    
    This time we chose to not use \textsc{FlowDroid}'s default configuration. First, we disabled static field tracking due to the global scope as described in \autoref{s:complexity}. Next, instead of the \code{EasyTaintWrapper} we use the \code{SummaryTaintWrapper} which utilizes \textsc{StubDroid}. \textsc{StubDroid}'s precomputed dataflow summaries are more precise than \code{EasyTaintWrapper}'s simple rules. The timeout for the dataflow analysis was set to 10 minutes. 
    The configuration is summarized in \autoref{t:realworldconfig}.
    ...
    Also, we did not use the full sources and sinks list included in \textsc{FlowDroid} because such would result in hundreds of sources and sinks per app and probably a really long runtime. Instead, we chose to analyze which sensitive, possibly user-identifying data is sent out to the internet. As we want to compare the forwards and backwards implementation, it is also important to not put one in a disadvantage. We opted for a 2:1 ratio of sources to sinks. This decision is based on the results of \textsc{SuSi}, a tool to automatically find sources and sinks in the Android framework \cite{Rasthofer2014}. Their extracted list of sources and sinks contains roughly $2.17$ times more sources than sinks.
    ...
    The point of comparison with \textsc{FlowDroid}'s forwards implementation was run using the to-date latest upstream commit from the develop branch\footnote{The latest upstream commit was at that time b436733fc4a5130dfe4ce8ddb3f76fd374e9a487.}. The backwards implementation ran on our latest commit with all changes merged from the upstream.

    For our app set, we chose 200 apps randomly out of an Google Playstore dump from 2021 containing over 6000 apps. The full list is appended to this work and can be found at \autoref{?}.
    ...
    \begin{table}[ht]
        ...
        \end{tabular}
        \caption{Timeout in the App Set}
    \end{table}
