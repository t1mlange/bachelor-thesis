...
\begin{document}
    ...
    \section{DroidBench}\label{s:droidbenchevaluation}
    ...
    \footnotetext{Outliers are runs with at least $25\%$ difference to the median run and a minimum of $5$ seconds absolute difference.}
    ...
    We used \textsc{FlowDroid}'s forward implementation on the latest upstream commit\footnote{The latest upstream commit was at that time \footnotecode{b436733fc4a5130dfe4ce8ddb3f76fd374e9a487}.} from the develop branch for the point of comparison.
    ...
    The existing implementation suffers as ours, so we suspect it partly depends on an unfortunately drawn app set, further development in the call graph generation and more summaries for the \code{SummaryTaintWrapper} leading to more possible edges.
    ...
    \footnotetext{If others are interested in finding the cause, \footnotecode{com.Slack} is a good starting point. It has the least edge propagations per ten minutes in our set.}
    ...
    This also confirmes that the default handling of sources and sinks does not bias the outcome.
    ...
    \Cref{t:memres} shows an overview of the results from the memory evaluation.
    ...
    \footnotetext{This is at least true for apps without a timeout.} 
